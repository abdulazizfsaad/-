...
export default async function CasesPage() {
  ...
  return (
    <>
      <PageHeader title="حالات التحصيل" description="إدارة دورة المتابعة من الحالة الجديدة حتى السداد أو التصعيد القانوني." />
      <div className="grid gap-4 xl:grid-cols-2">
        {cases.map((item) => (
          <Card key={item.id}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><p className="text-xs text-muted-foreground">معرف الحالة</p><h2 className="text-xl font-extrabold">{item.caseNumber}</h2></div>
